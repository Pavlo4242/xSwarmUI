...
// Paste this at the very end of main.js or include as separate file
// MINIMAL implementation - only adds, never modifies existing code

// 1. Toggle to split layout - bottom bar on sides, image in center
function toggleMaximizeImage() {
    let maximized = localStorage.getItem('maximize_image') == 'true';
    maximized = !maximized;
    localStorage.setItem('maximize_image', maximized);
    
    let style = document.getElementById('maximize_image_style');
    if (!style) {
        style = document.createElement('style');
        style.id = 'maximize_image_style';
        document.head.appendChild(style);
    }
    
    if (maximized) {
        style.textContent = `
            /* Hide metadata overlays */
            .image-metadata-overlay { display: none !important; }
            .image_metadata { display: none !important; }
            ...
            #bottom_info_bar {
                ...
                z-index: 50 !important;
                background: var(--main-bg-color, #1a1a1a) !important;
                padding: 10px !important;
            }
            
            #bottom_info_bar .bottom-info-bar-component {
                display: block !important;
    ...
    // Trigger layout reapply if available
    setTimeout(() => {
        if (window.genTabLayout && genTabLayout.reapplyPositions) {
...
// Apply on load
setTimeout(() => {
    if (localStorage.getItem('maximize_image') == 'true') {
        toggleMaximizeImage();
        toggleMaximizeImage(); // Call twice to set state correctly
    }
}, 1000);

// 2. Track recently used LoRAs
let loraRecentHistory = JSON.parse(localStorage.getItem('lora_recent_history') || '[]');

function trackLoraUsage(loraName) {
    try {
        let cleaned = loraName.replace('.safetensors', '').replace('.ckpt', '');
        loraRecentHistory = loraRecentHistory.filter(l => l !== cleaned);
        loraRecentHistory.unshift(cleaned);
        if (loraRecentHistory.length > 100) {
            loraRecentHistory = loraRecentHistory.slice(0, 100);
        }
        localStorage.setItem('lora_recent_history', JSON.stringify(loraRecentHistory));
    } catch (e) {
        console.log('Error tracking LoRA:', e);
    }
}

function showRecentLoras() {
    if (loraRecentHistory.length === 0) {
        alert('No recently used LoRAs yet!');
        return;
    }
    let list = loraRecentHistory.slice(0, 20).join('\n');
    alert('Recently used LoRAs:\n\n' + list);
}

// 3. Add +/- buttons to LoRA weights
function addWeightArrows(weightInput) {
    if (weightInput.dataset.arrowsAdded) return;
    weightInput.dataset.arrowsAdded = 'true';
    
    let step = 0.1;
    
    let decreaseBtn = document.createElement('button');
    decreaseBtn.innerHTML = '−';
    decreaseBtn.style.cssText = 'padding: 2px 6px; margin: 0 2px;';
    decreaseBtn.onclick = (e) => {
        e.preventDefault();
        e.stopPropagation();
        weightInput.value = (parseFloat(weightInput.value) - step).toFixed(1);
    ...
    increaseBtn.style.cssText = 'padding: 2px 6px; margin: 0 2px;';
    ...
    weightInput.parentNode.insertBefore(decreaseBtn, weightInput);
    weightInput.parentNode.insertBefore(increaseBtn, weightInput.nextSibling);
}

// Watch for LoRA weight inputs
setInterval(() => {
    let weightInputs = document.querySelectorAll('.lora-weight-input');
    weightInputs.forEach(input => {
        if (!input.dataset.arrowsAdded) {
            addWeightArrows(input);
        }
    });
}, 1000);

// Track LoRA usage (hook into existing system safely)
setTimeout(() => {
    if (window.loraHelper && loraHelper.selectLora) {
        let original = loraHelper.selectLora;
        loraHelper.selectLora = function(lora) {
            try {
                let name = (typeof lora === 'string') ? lora : (lora && lora.name) ? lora.name : '';
                if (name) trackLoraUsage(name);
            } catch (e) {}
            return original.call(this, lora);
        };
    }
}, 2000);

// Add to Quick Tools menu
setTimeout(() => {
    let popover = document.getElementById('popover_quicktools');
    if (popover && !popover.dataset.enhanced) {
        popover.dataset.enhanced = 'true';
        
        let div1 = document.createElement('div');
        div1.className = 'sui_popover_model_button';
        div1.onclick = toggleMaximizeImage;
        div1.innerText = 'Toggle Maximize Image';
        popover.appendChild(div1);
        
        let div2 = document.createElement('div');
        div2.className = 'sui_popover_model_button';
        div2.onclick = showRecentLoras;
        div2.innerText = 'Show Recent LoRAs';
        popover.appendChild(div2);
    }
}, 2000);
