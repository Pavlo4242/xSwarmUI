...
class MovableGenTab {
    ...
    /** Marks this tab as currently selected, visually showing it. */
    setSelected() {
...
class GenTabLayout {
    ...
    constructor() {
        ...
        // Register the prompt region as a pseudo-tab so it can be moved via buildConfigArea
        this.promptMovable = {
            id: 'alt_prompt_region',
            title: 'Prompt & Generate',
            contentElem: this.altRegion,
            targetGroupId: getCookie('tabloc_alt_prompt_region') || 'Input-Sidebar-Main-Tab',
            visible: true,
            update: () => {
                let target = getRequiredElementById(this.promptMovable.targetGroupId);
                if (target && this.altRegion.parentElement != target) {
                    target.appendChild(this.altRegion);
                    this.altRegion.style.position = 'relative';
                    this.altRegion.style.top = '0';
    ...
    reapplyPositions() {
        ...
        let curImgWidth = `calc(100vw - ${barTopLeft} - ${barTopRight} - 10px)`;
        ...
        if (imageEditor && imageEditor.active) {
            ...
            this.currentImage.style.width = curImgWidth;
        }
        this.currentImageWrapbox.style.width = curImgWidth;
    ...
    updateConfigFor(id) {
        ...
        if (tab) {
            let group = document.getElementById(`tabconfig_${id}_group`);
            let visible = document.getElementById(`tabconfig_${id}_visible`);
            if (group && visible) {
                tab.visible = visible.checked;
                tab.targetGroupId = group.value;
                tab.update();
                this.buildConfigArea();
            }
        }
    ...
    buildConfigArea() {
        ...
        for (let tab of this.managedTabs) {
            let group = document.getElementById(`tabconfig_${tab.id}_group`);
            let visible = document.getElementById(`tabconfig_${tab.id}_visible`);
            if (group) {
                group.value = tab.targetGroupId;
                group.addEventListener('change', () => this.updateConfigFor(tab.id));
            }
            if (visible) {
                visible.addEventListener('change', () => this.updateConfigFor(tab.id));
            }
        }
